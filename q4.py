...
class q4(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="3"):
					self.render('q4.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="adeltaarabt") and p.Level==3 ):
				...
				p.put()
				if( p.Level==3):
					self.render('q4.html',error="Incorrect",Tries=p.Tries)
				else:
