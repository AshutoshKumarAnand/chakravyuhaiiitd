...
class q15(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="14"):
					self.render('q15.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="universityofcaliforniaberkeley") and p.Level==14):
				...
				p.put()
				if( p.Level==14):
					self.render('q15.html',error="Incorrect",Tries=p.Tries)
				else:
