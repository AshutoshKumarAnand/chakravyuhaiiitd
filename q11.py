...
class q11(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="10"):
					self.render('q11.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="nokiatune") and p.Level==10 ):
				...
				p.put()
				if( p.Level==10):
					self.render('q11.html',error="Incorrect",Tries=p.Tries)
				else:
