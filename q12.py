...
class q12(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="11"):
					self.render('q12.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="castoroil") and p.Level==11 ):
				...
				p.put()
				if( p.Level==11):
					self.render('q12.html',error="Incorrect",Tries=p.Tries)
				else:
