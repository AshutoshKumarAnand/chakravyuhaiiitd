...
class q24(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="23"):
					self.render('q24.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="mohini") and p.Level==23):
				...
				p.put()
				if(p.Level==23):
					self.render('q24.html',error="Incorrect",Tries=p.Tries)
				else:
