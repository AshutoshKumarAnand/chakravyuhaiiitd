...
class q3(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="2"):
					self.render('q3.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if(act):		
			...
			if((string.lower(ans3)=="lasvegas") and p.Level==2 ):
			...
			#	p.Check+=1
				...
				p.put()
				if(p.Level==2):
					self.render('q3.html',error="Incorrect",Tries=p.Tries)
				else:
