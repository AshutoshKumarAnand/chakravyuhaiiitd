...
class q19(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="18"):
					self.render('q19.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="josefmengele") and p.Level==18):
				...
				p.put()
				if(p.Level==18):
					self.render('q19.html',error="Incorrect",Tries=p.Tries)
				else:
