...
class q22(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="21"):
					self.render('q22.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="stevejobs") and p.Level==21):
				...
				p.put()
				if(p.Level==21):
					self.render('q22.html',error="Incorrect",Tries=p.Tries)
				else:
