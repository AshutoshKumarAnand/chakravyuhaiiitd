...
class q20(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="19"):
					self.render('q20.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="delhi") and p.Level==19):
				...
				p.put()
				if(p.Level==19):
					self.render('q20.html',error="Incorrect",Tries=p.Tries)
				else:
