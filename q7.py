...
class q7(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="6"):
					self.render('q7.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="androidfour") and p.Level==6 ):
				...
				p.put()
				if( p.Level==6):
					self.render('q7.html',error="Incorrect",Tries=p.Tries)
				else:
