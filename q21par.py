...
class q21par(QuizHandler):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="20"):
					self.render('q21par.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="newyork") and p.Level==20):
				...
				p.put()
				if(p.Level==20):
					self.render('q21par.html',error="Incorrect",Tries=p.Tries)
				else:
