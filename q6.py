...
class q6(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="5"):
					self.render('q6.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="kevinspacey") and p.Level==5 ):
				...
				p.put()
				if( p.Level==5):
					self.render('q6.html',error="Incorrect",Tries=p.Tries)
				else:
