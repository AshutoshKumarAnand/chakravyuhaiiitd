...
class q14(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="13"):
					self.render('q14.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="thirteen") and p.Level==13):
				...
				p.put()
				if(p.Level==13):
					self.render('q14.html',error="Incorrect",Tries=p.Tries)
				else:
