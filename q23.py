...
class q23(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="22"):
					self.render('q23.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="charlesbabbage") and p.Level==22):
				...
				p.put()
				if(p.Level==22):
					self.render('q23.html',error="Incorrect",Tries=p.Tries)
				else:
