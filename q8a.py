...
class q8a(QuizHandler):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="7"):
					self.render('q8a.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="unbreakable") and p.Level==7 ):
				...
				p.put()
				if( p.Level==7):
					self.render('q8.html',error="Incorrect",Tries=p.Tries)
				else:
