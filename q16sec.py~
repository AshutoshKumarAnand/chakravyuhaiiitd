...
class q16sec(QuizHandler):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="15"):
					self.render('q16sec.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="gurudutt") and p.Level==15):
				...
				p.put()
				if( p.Level==15):
					self.render('q16sec.html',error="Incorrect",Tries=p.Tries)
				else:
