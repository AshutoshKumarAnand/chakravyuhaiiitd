...
class q16(QuizHandler):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="15"):
					self.render('q16.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="satyajitray") and p.Level==15):
				...
				p.put()
				if( p.Level==15):
					self.render('q16.html',error="Incorrect",Tries=p.Tries)
				else:
