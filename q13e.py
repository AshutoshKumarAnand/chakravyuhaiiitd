...
class q13e(QuizHandler):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="12"):
					self.render('q13e.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="792") and p.Level==12 ):
				...
				p.put()
				if( p.Level==12):
					self.render('q13e.html',error="Incorrect",Tries=p.Tries)
				else:
