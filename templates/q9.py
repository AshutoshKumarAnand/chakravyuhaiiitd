...
class q9(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[0]=="8"):
					self.render('q9.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="mrshudson") and p.Level==8 ):
				...
				p.put()
				if( p.Level==8):
					self.render('q9.html',error="Incorrect",Tries=p.Tries)
				else:
