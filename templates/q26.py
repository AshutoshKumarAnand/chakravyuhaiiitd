...
class q26(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="25"):
					self.render('q26.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="friends") and p.Level==25):
				...
				p.put()
				if(p.Level==24):
					self.render('q26.html',error="Incorrect",Tries=p.Tries)
				else:
