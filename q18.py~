...
class q18(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="17"):
					self.render('q18.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="jameswallaceburke") and p.Level==17):
				...
				p.put()
				if(p.Level==17):
					self.render('q18.html',error="Incorrect",Tries=p.Tries)
				else:
