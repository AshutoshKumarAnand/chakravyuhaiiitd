...
class q17(QuizHandler):
	def get(self):
		...
		if p_id:
			...
			if(checker(enc_name) and level_checker(enc_level,int(p_id))):
				if(checker(enc_level)[:2]=="16"):
					self.render('q17.html',error="",Tries=p.Tries)
				else:
	...
	def post(self):
		...
		if act:
			...
			if((string.lower(ans3)=="krazybosnians") and p.Level==16):
				...
				p.put()
				if(p.Level==16):
					self.render('q17.html',error="Incorrect",Tries=p.Tries)
				else:
